...
// =================================================================
// SE / BGM 管理（cloneNode再生方式）
// AudioContext は使用しない。全環境で確実に鳴らすことを最優先。
// SE再生: プリロード済み Audio を cloneNode() してから再生。
//   → スマホ再生制限回避・同時発音OK・audio競合なし・遅延軽減。
// BGM: 単一 Audio で loop 再生（変更なし）。
// =================================================================
...
const Sound = (() => {
  ...
  // SE を new Audio() でプリロード（再生はせず cloneNode 用のマスターとして保持）
  function _initSE() {
  ...
  /**
   * SE を再生する
   * cloneNode 方式: マスター Audio を複製して再生。
   * スマホ遅延・同時発音・連打すべてに対応。
   * @param {string} key
   */
  function se(key) {
